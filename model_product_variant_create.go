/*
SimpleBilly API

Simplebilly API - Bookkeeping, CRM, ERP. Multi-tenant API: a tenant is isolated and routed by subdomain (or a configured custom domain) under the base domain.  ## Rate limiting All endpoints are rate-limited per client IP: **100 requests per minute** on API routes and **5 requests per minute** on authentication routes. Exceeding a limit returns `429 Too Many Requests`; the window resets after 60 seconds.

API version: 0.1.0
Contact: support@simplebilly.com
*/

// Code generated by OpenAPI Generator (https://openapi-generator.tech); DO NOT EDIT.

package simplebillyapi

import (
	"encoding/json"
	"bytes"
	"fmt"
)

// checks if the ProductVariantCreate type satisfies the MappedNullable interface at compile time
var _ MappedNullable = &ProductVariantCreate{}

// ProductVariantCreate struct for ProductVariantCreate
type ProductVariantCreate struct {
	Barcode NullableString `json:"barcode,omitempty"`
	ImageLink NullableString `json:"imageLink,omitempty"`
	IsActive *bool `json:"isActive,omitempty"`
	// Human-readable variant label, e.g. \"Red / M\".
	Name NullableString `json:"name,omitempty"`
	// Option name → value map, e.g. `{\"Color\": \"Red\", \"Size\": \"M\"}`.
	OptionValues interface{} `json:"optionValues,omitempty"`
	// Explicit override price for this variant (takes precedence over parent price + delta).
	Price NullableString `json:"price,omitempty"`
	// Price adjustment relative to the parent product's `default_price`.
	PriceDelta NullableString `json:"priceDelta,omitempty"`
	// The parent product this variant belongs to. References the product entity.
	ProductId string `json:"productId"`
	// Variant-specific SKU (must be unique per tenant).
	Sku string `json:"sku"`
	// Variant-level stock (optional — may be tracked on the parent only).
	StockQuantity NullableInt64 `json:"stockQuantity,omitempty"`
}

type _ProductVariantCreate ProductVariantCreate

// NewProductVariantCreate instantiates a new ProductVariantCreate object
// This constructor will assign default values to properties that have it defined,
// and makes sure properties required by API are set, but the set of arguments
// will change when the set of required properties is changed
func NewProductVariantCreate(productId string, sku string) *ProductVariantCreate {
	this := ProductVariantCreate{}
	this.ProductId = productId
	this.Sku = sku
	return &this
}

// NewProductVariantCreateWithDefaults instantiates a new ProductVariantCreate object
// This constructor will only assign default values to properties that have it defined,
// but it doesn't guarantee that properties required by API are set
func NewProductVariantCreateWithDefaults() *ProductVariantCreate {
	this := ProductVariantCreate{}
	return &this
}

// GetBarcode returns the Barcode field value if set, zero value otherwise (both if not set or set to explicit null).
func (o *ProductVariantCreate) GetBarcode() string {
	if o == nil || IsNil(o.Barcode.Get()) {
		var ret string
		return ret
	}
	return *o.Barcode.Get()
}

// GetBarcodeOk returns a tuple with the Barcode field value if set, nil otherwise
// and a boolean to check if the value has been set.
// NOTE: If the value is an explicit nil, `nil, true` will be returned
func (o *ProductVariantCreate) GetBarcodeOk() (*string, bool) {
	if o == nil {
		return nil, false
	}
	return o.Barcode.Get(), o.Barcode.IsSet()
}

// HasBarcode returns a boolean if a field has been set.
func (o *ProductVariantCreate) HasBarcode() bool {
	if o != nil && o.Barcode.IsSet() {
		return true
	}

	return false
}

// SetBarcode gets a reference to the given NullableString and assigns it to the Barcode field.
func (o *ProductVariantCreate) SetBarcode(v string) {
	o.Barcode.Set(&v)
}
// SetBarcodeNil sets the value for Barcode to be an explicit nil
func (o *ProductVariantCreate) SetBarcodeNil() {
	o.Barcode.Set(nil)
}

// UnsetBarcode ensures that no value is present for Barcode, not even an explicit nil
func (o *ProductVariantCreate) UnsetBarcode() {
	o.Barcode.Unset()
}

// GetImageLink returns the ImageLink field value if set, zero value otherwise (both if not set or set to explicit null).
func (o *ProductVariantCreate) GetImageLink() string {
	if o == nil || IsNil(o.ImageLink.Get()) {
		var ret string
		return ret
	}
	return *o.ImageLink.Get()
}

// GetImageLinkOk returns a tuple with the ImageLink field value if set, nil otherwise
// and a boolean to check if the value has been set.
// NOTE: If the value is an explicit nil, `nil, true` will be returned
func (o *ProductVariantCreate) GetImageLinkOk() (*string, bool) {
	if o == nil {
		return nil, false
	}
	return o.ImageLink.Get(), o.ImageLink.IsSet()
}

// HasImageLink returns a boolean if a field has been set.
func (o *ProductVariantCreate) HasImageLink() bool {
	if o != nil && o.ImageLink.IsSet() {
		return true
	}

	return false
}

// SetImageLink gets a reference to the given NullableString and assigns it to the ImageLink field.
func (o *ProductVariantCreate) SetImageLink(v string) {
	o.ImageLink.Set(&v)
}
// SetImageLinkNil sets the value for ImageLink to be an explicit nil
func (o *ProductVariantCreate) SetImageLinkNil() {
	o.ImageLink.Set(nil)
}

// UnsetImageLink ensures that no value is present for ImageLink, not even an explicit nil
func (o *ProductVariantCreate) UnsetImageLink() {
	o.ImageLink.Unset()
}

// GetIsActive returns the IsActive field value if set, zero value otherwise.
func (o *ProductVariantCreate) GetIsActive() bool {
	if o == nil || IsNil(o.IsActive) {
		var ret bool
		return ret
	}
	return *o.IsActive
}

// GetIsActiveOk returns a tuple with the IsActive field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *ProductVariantCreate) GetIsActiveOk() (*bool, bool) {
	if o == nil || IsNil(o.IsActive) {
		return nil, false
	}
	return o.IsActive, true
}

// HasIsActive returns a boolean if a field has been set.
func (o *ProductVariantCreate) HasIsActive() bool {
	if o != nil && !IsNil(o.IsActive) {
		return true
	}

	return false
}

// SetIsActive gets a reference to the given bool and assigns it to the IsActive field.
func (o *ProductVariantCreate) SetIsActive(v bool) {
	o.IsActive = &v
}

// GetName returns the Name field value if set, zero value otherwise (both if not set or set to explicit null).
func (o *ProductVariantCreate) GetName() string {
	if o == nil || IsNil(o.Name.Get()) {
		var ret string
		return ret
	}
	return *o.Name.Get()
}

// GetNameOk returns a tuple with the Name field value if set, nil otherwise
// and a boolean to check if the value has been set.
// NOTE: If the value is an explicit nil, `nil, true` will be returned
func (o *ProductVariantCreate) GetNameOk() (*string, bool) {
	if o == nil {
		return nil, false
	}
	return o.Name.Get(), o.Name.IsSet()
}

// HasName returns a boolean if a field has been set.
func (o *ProductVariantCreate) HasName() bool {
	if o != nil && o.Name.IsSet() {
		return true
	}

	return false
}

// SetName gets a reference to the given NullableString and assigns it to the Name field.
func (o *ProductVariantCreate) SetName(v string) {
	o.Name.Set(&v)
}
// SetNameNil sets the value for Name to be an explicit nil
func (o *ProductVariantCreate) SetNameNil() {
	o.Name.Set(nil)
}

// UnsetName ensures that no value is present for Name, not even an explicit nil
func (o *ProductVariantCreate) UnsetName() {
	o.Name.Unset()
}

// GetOptionValues returns the OptionValues field value if set, zero value otherwise (both if not set or set to explicit null).
func (o *ProductVariantCreate) GetOptionValues() interface{} {
	if o == nil {
		var ret interface{}
		return ret
	}
	return o.OptionValues
}

// GetOptionValuesOk returns a tuple with the OptionValues field value if set, nil otherwise
// and a boolean to check if the value has been set.
// NOTE: If the value is an explicit nil, `nil, true` will be returned
func (o *ProductVariantCreate) GetOptionValuesOk() (*interface{}, bool) {
	if o == nil || IsNil(o.OptionValues) {
		return nil, false
	}
	return &o.OptionValues, true
}

// HasOptionValues returns a boolean if a field has been set.
func (o *ProductVariantCreate) HasOptionValues() bool {
	if o != nil && !IsNil(o.OptionValues) {
		return true
	}

	return false
}

// SetOptionValues gets a reference to the given interface{} and assigns it to the OptionValues field.
func (o *ProductVariantCreate) SetOptionValues(v interface{}) {
	o.OptionValues = v
}

// GetPrice returns the Price field value if set, zero value otherwise (both if not set or set to explicit null).
func (o *ProductVariantCreate) GetPrice() string {
	if o == nil || IsNil(o.Price.Get()) {
		var ret string
		return ret
	}
	return *o.Price.Get()
}

// GetPriceOk returns a tuple with the Price field value if set, nil otherwise
// and a boolean to check if the value has been set.
// NOTE: If the value is an explicit nil, `nil, true` will be returned
func (o *ProductVariantCreate) GetPriceOk() (*string, bool) {
	if o == nil {
		return nil, false
	}
	return o.Price.Get(), o.Price.IsSet()
}

// HasPrice returns a boolean if a field has been set.
func (o *ProductVariantCreate) HasPrice() bool {
	if o != nil && o.Price.IsSet() {
		return true
	}

	return false
}

// SetPrice gets a reference to the given NullableString and assigns it to the Price field.
func (o *ProductVariantCreate) SetPrice(v string) {
	o.Price.Set(&v)
}
// SetPriceNil sets the value for Price to be an explicit nil
func (o *ProductVariantCreate) SetPriceNil() {
	o.Price.Set(nil)
}

// UnsetPrice ensures that no value is present for Price, not even an explicit nil
func (o *ProductVariantCreate) UnsetPrice() {
	o.Price.Unset()
}

// GetPriceDelta returns the PriceDelta field value if set, zero value otherwise (both if not set or set to explicit null).
func (o *ProductVariantCreate) GetPriceDelta() string {
	if o == nil || IsNil(o.PriceDelta.Get()) {
		var ret string
		return ret
	}
	return *o.PriceDelta.Get()
}

// GetPriceDeltaOk returns a tuple with the PriceDelta field value if set, nil otherwise
// and a boolean to check if the value has been set.
// NOTE: If the value is an explicit nil, `nil, true` will be returned
func (o *ProductVariantCreate) GetPriceDeltaOk() (*string, bool) {
	if o == nil {
		return nil, false
	}
	return o.PriceDelta.Get(), o.PriceDelta.IsSet()
}

// HasPriceDelta returns a boolean if a field has been set.
func (o *ProductVariantCreate) HasPriceDelta() bool {
	if o != nil && o.PriceDelta.IsSet() {
		return true
	}

	return false
}

// SetPriceDelta gets a reference to the given NullableString and assigns it to the PriceDelta field.
func (o *ProductVariantCreate) SetPriceDelta(v string) {
	o.PriceDelta.Set(&v)
}
// SetPriceDeltaNil sets the value for PriceDelta to be an explicit nil
func (o *ProductVariantCreate) SetPriceDeltaNil() {
	o.PriceDelta.Set(nil)
}

// UnsetPriceDelta ensures that no value is present for PriceDelta, not even an explicit nil
func (o *ProductVariantCreate) UnsetPriceDelta() {
	o.PriceDelta.Unset()
}

// GetProductId returns the ProductId field value
func (o *ProductVariantCreate) GetProductId() string {
	if o == nil {
		var ret string
		return ret
	}

	return o.ProductId
}

// GetProductIdOk returns a tuple with the ProductId field value
// and a boolean to check if the value has been set.
func (o *ProductVariantCreate) GetProductIdOk() (*string, bool) {
	if o == nil {
		return nil, false
	}
	return &o.ProductId, true
}

// SetProductId sets field value
func (o *ProductVariantCreate) SetProductId(v string) {
	o.ProductId = v
}

// GetSku returns the Sku field value
func (o *ProductVariantCreate) GetSku() string {
	if o == nil {
		var ret string
		return ret
	}

	return o.Sku
}

// GetSkuOk returns a tuple with the Sku field value
// and a boolean to check if the value has been set.
func (o *ProductVariantCreate) GetSkuOk() (*string, bool) {
	if o == nil {
		return nil, false
	}
	return &o.Sku, true
}

// SetSku sets field value
func (o *ProductVariantCreate) SetSku(v string) {
	o.Sku = v
}

// GetStockQuantity returns the StockQuantity field value if set, zero value otherwise (both if not set or set to explicit null).
func (o *ProductVariantCreate) GetStockQuantity() int64 {
	if o == nil || IsNil(o.StockQuantity.Get()) {
		var ret int64
		return ret
	}
	return *o.StockQuantity.Get()
}

// GetStockQuantityOk returns a tuple with the StockQuantity field value if set, nil otherwise
// and a boolean to check if the value has been set.
// NOTE: If the value is an explicit nil, `nil, true` will be returned
func (o *ProductVariantCreate) GetStockQuantityOk() (*int64, bool) {
	if o == nil {
		return nil, false
	}
	return o.StockQuantity.Get(), o.StockQuantity.IsSet()
}

// HasStockQuantity returns a boolean if a field has been set.
func (o *ProductVariantCreate) HasStockQuantity() bool {
	if o != nil && o.StockQuantity.IsSet() {
		return true
	}

	return false
}

// SetStockQuantity gets a reference to the given NullableInt64 and assigns it to the StockQuantity field.
func (o *ProductVariantCreate) SetStockQuantity(v int64) {
	o.StockQuantity.Set(&v)
}
// SetStockQuantityNil sets the value for StockQuantity to be an explicit nil
func (o *ProductVariantCreate) SetStockQuantityNil() {
	o.StockQuantity.Set(nil)
}

// UnsetStockQuantity ensures that no value is present for StockQuantity, not even an explicit nil
func (o *ProductVariantCreate) UnsetStockQuantity() {
	o.StockQuantity.Unset()
}

func (o ProductVariantCreate) MarshalJSON() ([]byte, error) {
	toSerialize,err := o.ToMap()
	if err != nil {
		return []byte{}, err
	}
	return json.Marshal(toSerialize)
}

func (o ProductVariantCreate) ToMap() (map[string]interface{}, error) {
	toSerialize := map[string]interface{}{}
	if o.Barcode.IsSet() {
		toSerialize["barcode"] = o.Barcode.Get()
	}
	if o.ImageLink.IsSet() {
		toSerialize["imageLink"] = o.ImageLink.Get()
	}
	if !IsNil(o.IsActive) {
		toSerialize["isActive"] = o.IsActive
	}
	if o.Name.IsSet() {
		toSerialize["name"] = o.Name.Get()
	}
	if o.OptionValues != nil {
		toSerialize["optionValues"] = o.OptionValues
	}
	if o.Price.IsSet() {
		toSerialize["price"] = o.Price.Get()
	}
	if o.PriceDelta.IsSet() {
		toSerialize["priceDelta"] = o.PriceDelta.Get()
	}
	toSerialize["productId"] = o.ProductId
	toSerialize["sku"] = o.Sku
	if o.StockQuantity.IsSet() {
		toSerialize["stockQuantity"] = o.StockQuantity.Get()
	}
	return toSerialize, nil
}

func (o *ProductVariantCreate) UnmarshalJSON(data []byte) (err error) {
	// This validates that all required properties are included in the JSON object
	// by unmarshalling the object into a generic map with string keys and checking
	// that every required field exists as a key in the generic map.
	requiredProperties := []string{
		"productId",
		"sku",
	}

	allProperties := make(map[string]interface{})

	err = json.Unmarshal(data, &allProperties)

	if err != nil {
		return err;
	}

	for _, requiredProperty := range(requiredProperties) {
		if _, exists := allProperties[requiredProperty]; !exists {
			return fmt.Errorf("no value given for required property %v", requiredProperty)
		}
	}

	varProductVariantCreate := _ProductVariantCreate{}

	decoder := json.NewDecoder(bytes.NewReader(data))
	decoder.DisallowUnknownFields()
	err = decoder.Decode(&varProductVariantCreate)

	if err != nil {
		return err
	}

	*o = ProductVariantCreate(varProductVariantCreate)

	return err
}

type NullableProductVariantCreate struct {
	value *ProductVariantCreate
	isSet bool
}

func (v NullableProductVariantCreate) Get() *ProductVariantCreate {
	return v.value
}

func (v *NullableProductVariantCreate) Set(val *ProductVariantCreate) {
	v.value = val
	v.isSet = true
}

func (v NullableProductVariantCreate) IsSet() bool {
	return v.isSet
}

func (v *NullableProductVariantCreate) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullableProductVariantCreate(val *ProductVariantCreate) *NullableProductVariantCreate {
	return &NullableProductVariantCreate{value: val, isSet: true}
}

func (v NullableProductVariantCreate) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullableProductVariantCreate) UnmarshalJSON(src []byte) error {
	v.isSet = true
	return json.Unmarshal(src, &v.value)
}


