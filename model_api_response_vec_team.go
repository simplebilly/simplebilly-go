/*
SimpleBilly API

Simplebilly API - Bookkeeping, CRM, ERP. Multi-tenant API: a tenant is isolated and routed by subdomain (or a configured custom domain) under the base domain.  ## Rate limiting All endpoints are rate-limited per client IP: **100 requests per minute** on API routes and **5 requests per minute** on authentication routes. Exceeding a limit returns `429 Too Many Requests`; the window resets after 60 seconds.

API version: 0.1.0
Contact: support@simplebilly.com
*/

// Code generated by OpenAPI Generator (https://openapi-generator.tech); DO NOT EDIT.

package simplebillyapi

import (
	"encoding/json"
	"bytes"
	"fmt"
)

// checks if the ApiResponseVecTeam type satisfies the MappedNullable interface at compile time
var _ MappedNullable = &ApiResponseVecTeam{}

// ApiResponseVecTeam struct for ApiResponseVecTeam
type ApiResponseVecTeam struct {
	Data []ApiResponseTeamData `json:"data,omitempty"`
	Error NullableString `json:"error,omitempty"`
	Message NullableString `json:"message,omitempty"`
	Success bool `json:"success"`
}

type _ApiResponseVecTeam ApiResponseVecTeam

// NewApiResponseVecTeam instantiates a new ApiResponseVecTeam object
// This constructor will assign default values to properties that have it defined,
// and makes sure properties required by API are set, but the set of arguments
// will change when the set of required properties is changed
func NewApiResponseVecTeam(success bool) *ApiResponseVecTeam {
	this := ApiResponseVecTeam{}
	this.Success = success
	return &this
}

// NewApiResponseVecTeamWithDefaults instantiates a new ApiResponseVecTeam object
// This constructor will only assign default values to properties that have it defined,
// but it doesn't guarantee that properties required by API are set
func NewApiResponseVecTeamWithDefaults() *ApiResponseVecTeam {
	this := ApiResponseVecTeam{}
	return &this
}

// GetData returns the Data field value if set, zero value otherwise.
func (o *ApiResponseVecTeam) GetData() []ApiResponseTeamData {
	if o == nil || IsNil(o.Data) {
		var ret []ApiResponseTeamData
		return ret
	}
	return o.Data
}

// GetDataOk returns a tuple with the Data field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *ApiResponseVecTeam) GetDataOk() ([]ApiResponseTeamData, bool) {
	if o == nil || IsNil(o.Data) {
		return nil, false
	}
	return o.Data, true
}

// HasData returns a boolean if a field has been set.
func (o *ApiResponseVecTeam) HasData() bool {
	if o != nil && !IsNil(o.Data) {
		return true
	}

	return false
}

// SetData gets a reference to the given []ApiResponseTeamData and assigns it to the Data field.
func (o *ApiResponseVecTeam) SetData(v []ApiResponseTeamData) {
	o.Data = v
}

// GetError returns the Error field value if set, zero value otherwise (both if not set or set to explicit null).
func (o *ApiResponseVecTeam) GetError() string {
	if o == nil || IsNil(o.Error.Get()) {
		var ret string
		return ret
	}
	return *o.Error.Get()
}

// GetErrorOk returns a tuple with the Error field value if set, nil otherwise
// and a boolean to check if the value has been set.
// NOTE: If the value is an explicit nil, `nil, true` will be returned
func (o *ApiResponseVecTeam) GetErrorOk() (*string, bool) {
	if o == nil {
		return nil, false
	}
	return o.Error.Get(), o.Error.IsSet()
}

// HasError returns a boolean if a field has been set.
func (o *ApiResponseVecTeam) HasError() bool {
	if o != nil && o.Error.IsSet() {
		return true
	}

	return false
}

// SetError gets a reference to the given NullableString and assigns it to the Error field.
func (o *ApiResponseVecTeam) SetError(v string) {
	o.Error.Set(&v)
}
// SetErrorNil sets the value for Error to be an explicit nil
func (o *ApiResponseVecTeam) SetErrorNil() {
	o.Error.Set(nil)
}

// UnsetError ensures that no value is present for Error, not even an explicit nil
func (o *ApiResponseVecTeam) UnsetError() {
	o.Error.Unset()
}

// GetMessage returns the Message field value if set, zero value otherwise (both if not set or set to explicit null).
func (o *ApiResponseVecTeam) GetMessage() string {
	if o == nil || IsNil(o.Message.Get()) {
		var ret string
		return ret
	}
	return *o.Message.Get()
}

// GetMessageOk returns a tuple with the Message field value if set, nil otherwise
// and a boolean to check if the value has been set.
// NOTE: If the value is an explicit nil, `nil, true` will be returned
func (o *ApiResponseVecTeam) GetMessageOk() (*string, bool) {
	if o == nil {
		return nil, false
	}
	return o.Message.Get(), o.Message.IsSet()
}

// HasMessage returns a boolean if a field has been set.
func (o *ApiResponseVecTeam) HasMessage() bool {
	if o != nil && o.Message.IsSet() {
		return true
	}

	return false
}

// SetMessage gets a reference to the given NullableString and assigns it to the Message field.
func (o *ApiResponseVecTeam) SetMessage(v string) {
	o.Message.Set(&v)
}
// SetMessageNil sets the value for Message to be an explicit nil
func (o *ApiResponseVecTeam) SetMessageNil() {
	o.Message.Set(nil)
}

// UnsetMessage ensures that no value is present for Message, not even an explicit nil
func (o *ApiResponseVecTeam) UnsetMessage() {
	o.Message.Unset()
}

// GetSuccess returns the Success field value
func (o *ApiResponseVecTeam) GetSuccess() bool {
	if o == nil {
		var ret bool
		return ret
	}

	return o.Success
}

// GetSuccessOk returns a tuple with the Success field value
// and a boolean to check if the value has been set.
func (o *ApiResponseVecTeam) GetSuccessOk() (*bool, bool) {
	if o == nil {
		return nil, false
	}
	return &o.Success, true
}

// SetSuccess sets field value
func (o *ApiResponseVecTeam) SetSuccess(v bool) {
	o.Success = v
}

func (o ApiResponseVecTeam) MarshalJSON() ([]byte, error) {
	toSerialize,err := o.ToMap()
	if err != nil {
		return []byte{}, err
	}
	return json.Marshal(toSerialize)
}

func (o ApiResponseVecTeam) ToMap() (map[string]interface{}, error) {
	toSerialize := map[string]interface{}{}
	if !IsNil(o.Data) {
		toSerialize["data"] = o.Data
	}
	if o.Error.IsSet() {
		toSerialize["error"] = o.Error.Get()
	}
	if o.Message.IsSet() {
		toSerialize["message"] = o.Message.Get()
	}
	toSerialize["success"] = o.Success
	return toSerialize, nil
}

func (o *ApiResponseVecTeam) UnmarshalJSON(data []byte) (err error) {
	// This validates that all required properties are included in the JSON object
	// by unmarshalling the object into a generic map with string keys and checking
	// that every required field exists as a key in the generic map.
	requiredProperties := []string{
		"success",
	}

	allProperties := make(map[string]interface{})

	err = json.Unmarshal(data, &allProperties)

	if err != nil {
		return err;
	}

	for _, requiredProperty := range(requiredProperties) {
		if _, exists := allProperties[requiredProperty]; !exists {
			return fmt.Errorf("no value given for required property %v", requiredProperty)
		}
	}

	varApiResponseVecTeam := _ApiResponseVecTeam{}

	decoder := json.NewDecoder(bytes.NewReader(data))
	decoder.DisallowUnknownFields()
	err = decoder.Decode(&varApiResponseVecTeam)

	if err != nil {
		return err
	}

	*o = ApiResponseVecTeam(varApiResponseVecTeam)

	return err
}

type NullableApiResponseVecTeam struct {
	value *ApiResponseVecTeam
	isSet bool
}

func (v NullableApiResponseVecTeam) Get() *ApiResponseVecTeam {
	return v.value
}

func (v *NullableApiResponseVecTeam) Set(val *ApiResponseVecTeam) {
	v.value = val
	v.isSet = true
}

func (v NullableApiResponseVecTeam) IsSet() bool {
	return v.isSet
}

func (v *NullableApiResponseVecTeam) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullableApiResponseVecTeam(val *ApiResponseVecTeam) *NullableApiResponseVecTeam {
	return &NullableApiResponseVecTeam{value: val, isSet: true}
}

func (v NullableApiResponseVecTeam) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullableApiResponseVecTeam) UnmarshalJSON(src []byte) error {
	v.isSet = true
	return json.Unmarshal(src, &v.value)
}


